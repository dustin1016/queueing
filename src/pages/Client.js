...
export default function Client()
{
  ...
  const fetchQueue = async () => {
    try {
      const response = await fetch('http://10.125.1.10/data.php/getQueue');
  ...
  async function serveNumber(index) {
    try {
      const response = await fetch('http://10.125.1.10/data.php/serve',
       {
        method: 'POST',
        headers: {
  ...
  async function insertData(purpose)  {
    ...
    try {
      const response = await fetch('http://10.125.1.10/data.php/insert',
       {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
