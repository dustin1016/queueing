...
const Faculty = () => {
    ...
    const fetchList = async()=> {
        try {
            const response = await fetch('http://10.125.1.10/qsrv/data.php/faculties');
      ...
      const FacultyListCard = React.memo(({ id, name, img }) => {
        const imgpath = img !== null ? "http://10.125.1.10/qsrv/img/"+img : 'images/blank.webp';
      ...
      function SelectedFaculty(){
          ...
          const imgpath = selectedFaculty !== null ? "http://10.125.1.10/qsrv/img/"+selectedFaculty?.pic_name : 'images/blank.webp';
        return(
